...
function defineSymbol(c: Checker, name: string, kind: SymbolKind, span: Span, type?: Type, mutable?: boolean): void {
  ...
  if (c.currentScope.symbols.has(name)) {
    ...
    addError(c, span, `Duplicate definition of '${name}'. It was already defined at line ${getLine(c, existing.span)}.`);
    return;
  }
...
function checkExpr(c: Checker, expr: Expr): Type | undefined {
  switch (expr.kind) {
    ...
    case 'MemberExpr': {
      ...
      // Handle typed pointer dereference: ptr.u32, ptr.u8, etc.
      // This reads from memory at the pointer location as the specified type
      if (objType.kind === 'PointerType') {
        const numericTypes = ['i32', 'u32', 'i64', 'u64', 'f32', 'f64', 'u8', 'u16', 'i8', 'i16'];
        if (numericTypes.includes(expr.member)) {
          return { kind: 'PrimitiveType', name: expr.member, span: expr.span };
        }
      }
  ...
  switch (stmt.kind) {
    ...
    case 'Assignment': {
      ...
      for (let i = 0; i < stmt.targets.length; i++) {
        ...
        // For compound assignment (+=, -=, etc.), check if the operation is valid
        // rather than checking if the value type matches the target type
        if (stmt.op && expectedType && sym.type) {
          // Compound assignment: ptr += int is valid (pointer arithmetic)
          const baseOp = stmt.op.slice(0, -1); // Remove '=' from '+=' etc.
          if ((baseOp === '+' || baseOp === '-') && sym.type.kind === 'PointerType' && isIntegerType(expectedType)) {
...
function checkFunction(c: Checker, func: FuncDecl): void {
  ...
  // Register named returns as locals (skip if same name as parameter)
  if (func.returnType && 'params' in func.returnType) {
    const namedReturn = func.returnType as { params: Param[] };
    for (const ret of namedReturn.params) {
      // Skip if a parameter with this name already exists
      if (!c.currentScope.symbols.has(ret.name)) {
    ...
    if (exp.decl.kind === 'FuncDecl') {
      ...
      if (func.returnType) {
        if ('params' in func.returnType) {
          // Named returns - unwrap single-element tuples
          const namedReturn = func.returnType as { params: Param[]; span: Span };
          if (namedReturn.params.length === 1) {
            returnType = namedReturn.params[0].type;
          } else {
            returnType = {
              kind: 'TupleType',
              elements: namedReturn.params.map(p => p.type),
              span: namedReturn.span,
            };
          }
        } else {
